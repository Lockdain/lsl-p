...
class TimeSpec extends AnyWordSpec with Matchers {
  ...
  "Calling fromJson" should {
    ...
    "return Some wrapping a properly initialized com.lightbend.training.scalatrain.Time for a valid JSON object" in {
      Time fromJson JsObject(Seq("hours" -> JsNumber(9), "minutes" -> JsNumber(30))) shouldEqual Some(Time(9, 30))
    }
    "return Some wrapping a properly initialized com.lightbend.training.scalatrain.Time for a valid JSON object with hours only" in {
      Time fromJson JsObject(Seq("hours" -> JsNumber(9))) shouldEqual Some(Time(9, 0))
    }
  }

  "Calling fromJson after toJson" should {
    "return Some wrapping the original com.lightbend.training.scalatrain.Time" in {
      val time = Time(9, 30)
      (Time fromJson time.toJson) shouldEqual Some(time)
    }
  }

  "Creating a com.lightbend.training.scalatrain.Time" should {
